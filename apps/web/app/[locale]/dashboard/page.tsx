      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: '28px' }}>
        ...
        >
          ...
          >
            ...
            <span style={{ fontSize: '0.8rem', color: '#94a3b8', fontWeight: 500 }}>VFS Global</span>
            {vfsHealth ? (
              <span
                style={{
                  display: 'flex',
            ...
            return (
              ...
              >
                ...
                <div style={{ fontSize: '2rem', fontWeight: 700, color: kpi.color, lineHeight: 1 }}>
                  {isLoading ? '...' : formatNumber(kpi.value, locale)}
                </div>
              </div>
            );
          })}
              ...
              >
                <thead>
                  <tr style={{ borderBottom: '1px solid var(--border-subtle)', color: '#64748b' }}>
                    <th style={{ padding: '10px 12px', fontWeight: 500 }}>{tCommon('appName')}</th>
                    <th style={{ padding: '10px 12px', fontWeight: 500 }}>{tCommon('provider')}</th>
                    <th style={{ padding: '10px 12px', fontWeight: 500 }}>{tCommon('status')}</th>
                    <th style={{ padding: '10px 12px', fontWeight: 500 }}>{tCommon('actions')}</th>
                    <th style={{ padding: '10px 12px', fontWeight: 500 }}>{tCommon('created')}</th>
                    <th style={{ padding: '10px 12px', fontWeight: 500, textAlign: 'end' }}>{tCommon('actions')}</th>
                  </tr>
                </thead>
                <tbody>
                  {summary.attentionItems.map((item) => {
                    const statusConfig = getStatusConfig(item.currentStatus, locale);
                    return (
                      <tr
                        key={item.caseId}
                      ...
                      >
                        <td style={{ padding: '14px 12px', fontWeight: 600, color: '#f8fafc' }}>
                          ...
                          >
                            <TechnicalText>{item.caseNumber}</TechnicalText>
                          </Link>
                        </td>
                        ...
                        <td style={{ padding: '14px 12px', color: '#f59e0b', fontWeight: 600 }}>
                          {formatAgeSeconds(item.ageSeconds, locale)}
                        </td>
                        <td style={{ padding: '14px 12px', textAlign: 'end' }}>
                          ...
                          >
                            <span>{tCommon('viewDetails')}</span>
                            <ArrowUpRight size={12} className="icon-directional" />
                          </Link>
                        </td>
                      </tr>
                    );
                  ...
                  <div style={{ display: 'flex', flexDirection: 'column', gap: '2px' }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                      {activity.caseNumber && (
                        ...
                        >
                          <TechnicalText>{activity.caseNumber}</TechnicalText>
                        </Link>
                      )}
